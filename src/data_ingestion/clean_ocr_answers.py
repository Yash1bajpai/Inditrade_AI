...
def strip_letterhead(text: str) -> str:
    if not text:
        return ""
    # Comprehensive letterhead stripping
    cleaned = re.sub(
...
def extract_clean_subject_and_body(context_snippet: str) -> tuple:
    ...
    # Extra safety: make sure subject doesn't start with leftover header terms
...
def run_cleanup():
    # We load from git HEAD (the 1604 rows before our first cleanup pass today) so our counts match exactly from the original 1604 rows!
    ...
    for row in all_rows:
        if row.get("doc_type") == "DGFT_OCR" and any(kw in row.get("answer", "") for kw in keywords):
            ...
            # Double check that no keywords survived in subject or body
            ...
            # Final verification that no letterhead keywords exist in new_ans
