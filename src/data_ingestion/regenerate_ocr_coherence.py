...
def check_coherence(ans: str) -> list:
    ...
    # 1. Structural fragments & boilerplate preamble leaks
    ...
    # 3. Mid-sentence cutoff or cutoff right after preposition/article/word
...
def fix_ocr_garble(text: str) -> str:
    if not text:
        return ""
    t = text
    # Fix common date garble from scanned quotes/digits
    ...
    # Strip table column headers if they appear mid-sentence
    ...
    # Strip formal legal preamble boilerplate completely (whether full or cut off)
    t = re.sub(r'(?i)\bS\.?O\.?\s*\(E\)\.?[\s:\-]*', ' ', t)
    t = re.sub(r'(?i)\bIn\s+exercise\s+of\s+powers\s+conferred\b.*$', ' ', t)
    
    # Strip leading/trailing bullets and quotes
...
def extract_clean_subject(context_snippet: str) -> str:
    ...
    # Fallback to first meaningful sentence after letterhead
...
def synthesize_clean_answer(row: dict) -> str:
    ...
    if "primary subject and policy mandate" in question:
        ...
        if operative and len(operative.split()) >= 12:
            ...
            # If no complete procedural sentences exist inside context_snippet for Q2, mark as fragmentary so it gets deleted
            return ""
...
def run_coherence_regeneration():
    # Load from the state right before coherence check (1598 rows)
    ...
    for row in all_rows:
        if row.get("doc_type") == "DGFT_OCR":
            ...
            if reasons:
                ...
                # Verify regenerated answer passes coherence check
